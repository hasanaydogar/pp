...
import { compareWithBenchmark } from '@/lib/api/benchmark';

/**
 * GET /api/portfolios/[portfolioId]/benchmark-comparison
...
export async function GET(
  ...
  try {
    ...
    try {
      const comparison = await compareWithBenchmark(supabase, id);
      return NextResponse.json({ data: comparison });
    } catch (error) {
      if (error instanceof Error) {
        return internalServerError(error.message);
      }
      return internalServerError('Failed to compare with benchmark');
    }
  } catch (error) {
